...
def _choose_agg_type(state: GraphState) -> str:
    ...
    if interpreted.get("drug_name_2"):
        return "by_phase_per_drug"
    ...
    if any(kw in query for kw in ("year", "since", "trend", "over time", "timeline", "change", "over the")):
        return "by_year"
    ...
    if interpreted.get("start_year") and interpreted.get("end_year"):
        return "by_year"
...
def aggregate_data(state: GraphState) -> dict[str, Any]:
    ...
    if agg_type == "by_phase_per_drug":
        # Records are tagged dicts with comparison_drug — skip NormalizedTrialRecord validation.
        data = agg.count_by_phase_per_drug(raw_records, citation_limit=limit)
    else:
        records = _RecordAdapter.validate_python(raw_records)
        if agg_type == "by_year":
            from_year = state.get("start_year") or (
                (state.get("interpreted") or {}).get("start_year")
            )
