...
│    interpret_question   →  plan_tool_calls                  │
│    execute_tool_calls   →  assess_data_sufficiency          │
│    repair_plan (≤2x)    →  aggregate_data                   │
│    generate_visualization_spec  →  generate_insight         │
│    validate_response                                        │
│                                                             │
│  Services                                                   │
...
└──────────────┬──────────────────────────┬───────────────────┘
  ...
  (ClinicalTrials.gov API v2)      Claude Sonnet 4.6
```
...
2. `plan_tool_calls` — decides which ClinicalTrials.gov API calls to make, which filters to apply, and which aggregation strategy to use.
3. `execute_tool_calls` — calls the `ClinicalTrialsToolInvoker` against the live API. No LLM involved.
4. `assess_data_sufficiency` — pure Python: flags insufficient if fewer than 3 records were returned. No LLM. Routes to repair or aggregate.
5. `repair_plan` — doubles `max_records` per call and retries. Capped at two repair attempts.
6. `aggregate_data` — counts, groups, ranks, and builds network edges in **pure Python**. The LLM never counts records.
7. `generate_visualization_spec` — builds the final visualization JSON, selects chart encoding, and attaches per-datum citations.
8. `generate_insight` — generates a 2–3 sentence plain-English observation about the data using Claude Sonnet.
...
- **AWS Bedrock access is required.** Interpreting and planning are LLM-driven; both nodes fall back gracefully if the LLM is unavailable, but the result quality degrades to heuristic-only matching.
...
| `plan_tool_calls` | Multi-step routing with few-shot examples | Sonnet 4.6 | Sonnet 4.6 (keep) |
...
`plan_tool_calls` stays on Sonnet because it must reason over 12 aggregation types, 9 few-shot examples, and multi-call comparison plans — this is where query-routing errors are most costly.
